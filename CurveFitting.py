...
class DataTable(ttk.Treeview):
    ...
    def _draw_table(self, dataframe):
        ...
        for col in columns:
            ...
            self.column(col, width=int(self.winfo_width() * 0.5), anchor="center")  # 每列占窗口宽度的50%
    ...
    def find_value(self, pairs):
        ...
        # 如果结果为空则显示空表格
        if new_df.empty:
            self.delete(*self.get_children())  # 清空表格内容
            return None
...
class SearchPage(tk.Frame):
    def __init__(self, parent):
        ...
        self.min_label = tk.Label(parent, text="a.Min:\nb.Min:\nc.Min:\nd.Min:\nm.Min:\n")
        self.min_label.place(relx=0.3, rely=0.7, relwidth=0.05, relheight=0.2)
        
        self.max_label = tk.Label(parent, text="a.Max:\nb.Max:\nc.Max:\nd.Max:\nm.Max:\n")
        self.max_label.place(relx=0.45, rely=0.7, relwidth=0.05, relheight=0.2)
        
        
        self.a_min_entrybox = tk.Entry(parent, relief='solid', borderwidth=1)
        self.a_min_entrybox.insert(0,-1)
        self.a_min_entrybox.place(relx=0.35, rely=0.71, relwidth=0.075, relheight=0.025)
        
        self.b_min_entrybox = tk.Entry(parent, relief='solid', borderwidth=1)
        self.b_min_entrybox.insert(0,-np.inf)
        self.b_min_entrybox.place(relx=0.35, rely=0.74, relwidth=0.075, relheight=0.025)
        
        self.c_min_entrybox = tk.Entry(parent, relief='solid', borderwidth=1)
        self.c_min_entrybox.insert(0,0)
        self.c_min_entrybox.place(relx=0.35, rely=0.77, relwidth=0.075, relheight=0.025)
        
        self.d_min_entrybox = tk.Entry(parent, relief='solid', borderwidth=1)
        self.d_min_entrybox.insert(0,-np.inf)
        self.d_min_entrybox.place(relx=0.35, rely=0.80, relwidth=0.075, relheight=0.025)
        
        self.m_min_entrybox = tk.Entry(parent, relief='solid', borderwidth=1)
        self.m_min_entrybox.insert(0,-np.inf)
        self.m_min_entrybox.place(relx=0.35, rely=0.83, relwidth=0.075, relheight=0.025)
        
        
        self.a_max_entrybox = tk.Entry(parent, relief='solid', borderwidth=1)
        self.a_max_entrybox.insert(0,1)
        self.a_max_entrybox.place(relx=0.50, rely=0.71, relwidth=0.075, relheight=0.025)
        
        self.b_max_entrybox = tk.Entry(parent, relief='solid', borderwidth=1)
        self.b_max_entrybox.insert(0,np.inf)
        self.b_max_entrybox.place(relx=0.50, rely=0.74, relwidth=0.075, relheight=0.025)
        
        self.c_max_entrybox = tk.Entry(parent, relief='solid', borderwidth=1)
        self.c_max_entrybox.insert(0,np.inf)
        self.c_max_entrybox.place(relx=0.50, rely=0.77, relwidth=0.075, relheight=0.025)
        
        self.d_max_entrybox = tk.Entry(parent, relief='solid', borderwidth=1)
        self.d_max_entrybox.insert(0,np.inf)
        self.d_max_entrybox.place(relx=0.50, rely=0.80, relwidth=0.075, relheight=0.025)
        
        self.m_max_entrybox = tk.Entry(parent, relief='solid', borderwidth=1)
        self.m_max_entrybox.insert(0,np.inf)
        self.m_max_entrybox.place(relx=0.50, rely=0.83, relwidth=0.075, relheight=0.025)
        
        
        # Label for XData
        self.xdata_label = tk.Label(parent, text="XData:")
        self.xdata_label.place(relx=0, rely=0.2, relwidth=0.05, relheight=0.05)
        
        # Input box for XData
        self.xdata_entrybox = tk.Entry(parent, relief='solid', borderwidth=1)
        self.xdata_entrybox.place(relx=0.05, rely=0.2, relwidth=0.075, relheight=0.05)

        # Label for YData
        self.ydata_label = tk.Label(parent, text="YData:")
        self.ydata_label.place(relx=0.125, rely=0.2, relwidth=0.05, relheight=0.05)
        
        # Input box for YData
        self.ydata_entrybox = tk.Entry(parent, relief='solid', borderwidth=1)
        self.ydata_entrybox.place(relx=0.175, rely=0.2, relwidth=0.075, relheight=0.05)

        # Treeview
        self.data_table = DataTable(parent)
        self.data_table.place(relx=0, rely=0.3, relwidth=0.25, relheight=0.65)  # 调整DataTable的大小
        ...
        self.clear_button = tk.Button(parent, text=' File List Clear ', command=self.clear_file_list, relief='groove')
        self.clear_button.place(relx=0.05, rely=0.95, relwidth=0.15, relheight=0.05)

        # Start button to trigger fitting
        self.start_button = tk.Button(parent, text=' Start ', command=self.plot_5pl_curve, relief='groove')
        ...
        # Add the canvas for matplotlib to embed it into tkinter
        self.canvs = FigureCanvasTkAgg(self.fig, parent)
        self.canvs.get_tk_widget().place(relx=0.3, rely=0, relwidth=0.65, relheight=0.65)

        # Label for displaying result parameters
        self.result_label = tk.Label(parent, text="a=\nb=\nc=\nd=\nm=\nR2=")
        self.result_label.place(relx=0.75, rely=0.7, relwidth=0.05, relheight=0.2)

        # Label to show the fitted parameters (instead of tk.Text)
        self.result_value_label = tk.Label(parent, text="0.0000000000\n0.0000000000\n0.0000000000\n0.0000000000\n0.0000000000\n0.0000000000", justify="left", anchor="w")
        self.result_value_label.place(relx=0.8, rely=0.7, relwidth=0.15, relheight=0.2)

        # Copy button to copy the result to clipboard
        self.copy_button = tk.Button(parent, text=' Copy Result Parameter ', relief='groove',command=self.copy_result_to_clipboard)
